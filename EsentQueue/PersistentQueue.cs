...
public class PersistentQueue<T> : IDisposable
{
   ...
   public int Count
   {
      get
      {
         QueueCursor cursor = _cursorCache.GetCursor ( );
         try
         {
            int count = 0;
   ...
   public void Enqueue ( T item )
   {
      ...
      try
      {
         lock ( _instance )
         {
            ...
            using ( Update update = cursor.CreateDataTableUpdate ( ) )
            {
               using ( ColumnStream colStream = new ( cursor.Session, cursor.DataTable, cursor.SerializedObjectColumn ) )
               {
   ...
   public bool TryDequeue ( out T item )
   {
      ...
      try
      {
         lock ( _instance )
         {
            using Transaction tx = cursor.BeginTransaction ( );
   ...
   private void CheckDatabase ( )
   {
      ...
      using Session session = new ( _instance );
   ...
   private void CreateDatabase ( )
   {
      ...
      using Transaction transaction = new ( session );
